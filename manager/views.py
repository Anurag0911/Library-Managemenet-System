...
def trans():
    ...
    if request.method == 'POST':
        ...
        mem_id=Members.query.filter_by(name=mem_name).first()
        book_id=Books.query.filter_by(title=book_name).first()
      
        if len(iss_date) < 1:
            flash('name is too short!', category='error')
       
        else:
            new_trans = Trans(transby=book_id, transfor= mem_id, member_name=mem_name ,book_name=book_name ,iss_date=iss_date,ret_date=ret_date,rent = rent,fine = fine , user_id=current_user.id)
    ...
    return render_template("trans.html", user=current_user,books=books, trans=trans,members=members)
